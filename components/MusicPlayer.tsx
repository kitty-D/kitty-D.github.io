...
import { Music, ChevronDown, Play, Pause, SkipForward, SkipBack, Volume2, VolumeX } from 'lucide-react';
  ...
  return (
    <div className="fixed right-6 bottom-6 z-50 flex flex-col items-end gap-4 pointer-events-none">
      ...
      <div className="pointer-events-auto relative" ref={playerRef}>
        <div className={`relative transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isOpen && !isClosing ? 'w-[320px] min-h-[220px] opacity-100' : 'w-12 h-12 opacity-100'}`}>
          {!isOpen || isClosing ? (
            <button
          ...
          {isOpen && (
            <div className={`absolute bottom-0 right-0 backdrop-blur-3xl bg-black/10 shadow-2xl rounded-3xl p-4 pb-5 w-full min-h-full origin-bottom-right overflow-hidden transition-all duration-500 ease-[cubic-bezier(0.34,1.56,0.64,1)] ${isClosing ? 'scale-[0.15] translate-x-2 translate-y-2 rounded-[100px] opacity-0' : 'scale-100 translate-x-0 translate-y-0 opacity-100'}`}>
              ...
              <div className="mb-1.5 relative z-10">
                <h3 className="font-bold text-xl text-white truncate leading-tight mb-0.5 drop-shadow-lg">
                  {currentSong.title}
                </h3>
                <p className="text-xs text-white/80 truncate font-medium drop-shadow-md">
                  {currentSong.artist}
                </p>
              </div>

              {/* Controls */}
              <div className="flex flex-col gap-1 relative z-10 mb-1.5">
                {/* Progress Bar & Time */}
                ...
                <div className="flex items-center justify-between gap-2 mt-auto pt-2">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <button onClick={handlePlayPause} className="w-11 h-11 bg-white/20 hover:bg-white/30 text-white rounded-full flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-2xl backdrop-blur-xl border border-white/30 relative overflow-hidden">
                      {showLoadingUI && isPlaying ? (
