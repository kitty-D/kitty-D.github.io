...
const CategoryBadge = ({ category, language }: { category: Category; language: Language }) => (
  <span className="absolute top-4 left-4 z-30 inline-flex items-center rounded-full border border-white/35 bg-white/60 px-3 py-1.5 text-[11px] font-semibold leading-none tracking-[0.04em] text-[#111] shadow-[0_2px_10px_rgba(0,0,0,0.08),inset_0_1px_0_rgba(255,255,255,0.35)] backdrop-blur-2xl [-webkit-backdrop-filter:blur(16px)]">
...
export const PortfolioSection: React.FC<PortfolioSectionProps> = ({ language, externalFilter }) => {
  ...
  const availableCategories = preferredOrder;
        ...
        {filteredProjects.map((project) => (
          ...
          >
            ...
            {project.category === Category.DEV ? (
               ...
               <>
                  {/* Image container */}
                  <div className={`w-full ${(project.id === 'teamwork' || project.id === 'ljsk' || project.id === 'gothic-eye-tracking' ? 'aspect-[2/1]' : 'aspect-video')} bg-gray-100 dark:bg-gray-800 mb-6 overflow-hidden rounded-2xl`}>
                     ...
                     <div className="p-6 md:p-12">
                                ...
                                <div className="space-y-4 flex-1 min-w-[200px]">
                                    ...
                                    <div className="flex flex-row flex-wrap gap-x-4 gap-y-2">
                                        {displayProject.tags.map((tag) => (
                                            <span
                                                key={tag}
                                                className="text-sm font-bold font-mono text-gray-500 dark:text-gray-400 hover:text-black dark:hover:text-white transition-colors cursor-default"
                                            >
                                                #{tag}
                                            </span>
                                        ))}
                       ...
                       {displayProject.gallery && displayProject.gallery.length > 0 && (
                         <div className="space-y-6 md:space-y-8 mt-8 md:mt-12">
                           {displayProject.gallery.map((img, index) => (
                             <img
                               key={index}
                               src={img}
                               alt={`${displayProject.title}-${index}`}
